...
function checkEdifici() {
    console.log("check")
    let edifici = document.getElementsByName("edifici")[0];
    const correct = "✅";
    const incorrect = "❌";

    var check = document.getElementsByName("check"); // Variable amb imatge incorrecte/correcte
    let regex, primersCaracters = false; // Variables bool, per el check del input

    check[4].textContent = incorrect;
    const reg = /^[a-z/A-Z]{5}[+][0-9]{3}[.][a-z]{2}$/; //
    splitedCode = edifici.value.split("+");


    /* ----- Comprovació 3 lletres inicials de la familia coincideixen -----*/
    poblacions = document.getElementsByName("poblacions")
    const fam = poblacions[0].value.substring(0, 5);
    if (fam.toUpperCase() == splitedCode[0].toUpperCase()) {
        primersCaracters = true;
        console.log("5 LETT OK")
    } else {
        console.log("false")
    }
    regex = (reg.test(edifici.value));
    ...
    if (primersCaracters && regex ){
        check[4].textContent = correct;
        return true;
    } 
...
window.onload = function start() {
    ...
    /* ----- Poblacions ----- */
    var poblacions = ["Girona", "Madrid", "Barcelona", "Lleida", "Mallorca"];
    poblacions.sort(); // Ordena array alfabeticament
    var selectPoblacions = document.getElementsByName("poblacions")[0];
    //selectPoblacions.addEventListener("change", checkEdifici);
    generateDropBox(poblacions, selectPoblacions);

    /* ----- Edifici ----- */
    var edifici = document.getElementsByName("edifici");
    edifici[0].addEventListener("input", checkEdifici);
